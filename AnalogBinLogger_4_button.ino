...
const int button2 = 7; // pin7 will be used for recording data
const int LED1 = 8; // recording indication LED
const int LED2 = 4; // Conversion indication LED
const int DOUT = 9; //device output pin
const int DOUT1 = 2; //device1 output pin

unsigned long tim;
int state=LOW;
int gap=500; //gap in milliseconds
...
void logData() {
  ...
  while (1) {
    ...
    if (Serial.available() || digitalRead(button2)==LOW) {
      // Stop ISR calls.
      adcStop();
...
// while (Serial.available() && Serial.read() >= 0);
  ...
  Serial.println(F("c - convert file to csv OR press button-1"));
  Serial.println(F("d - dump data to Serial"));
  Serial.println(F("e - overrun error details"));
  Serial.println(F("r - record ADC data OR Press button-2"));


  while(!Serial.available() && read2!=HIGH) 
  {
